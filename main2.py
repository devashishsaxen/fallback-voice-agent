...
def get_audio(audio_id: str):
    log(f"=== AUDIO REQUEST ===")
    log(f"Requested: {audio_id}")
    audio_path = TEMP_AUDIO_DIR / audio_id
    
    if audio_path.exists():
        size = audio_path.stat().st_size
        log(f"Serving WAV file ({size} bytes)")
        return FileResponse(audio_path, media_type="audio/wav")
    else:
        log(f"ERROR: File not found!")
        available = [f.name for f in TEMP_AUDIO_DIR.iterdir() if f.is_file()]
...
async def twilio_webhook(request: Request, session_id: str = None, RecordingUrl: str = None, CallStatus: str = None):
    ...
    log(f"RecordingUrl: {RecordingUrl}")
    
    response = VoiceResponse()
    
    try:
        if not session_id or session_id not in sessions:
            log(f"ERROR: Session not found!")
        ...
        if audio_url:
            log(f"Playing audio: {audio_url}")
            response.play(audio_url)
        else:
            log("Using fallback TTS")
...
async def call_status(session_id: str = None, CallStatus: str = None):
    log(f"Call status for {session_id}: {CallStatus}")
    return {"status": "ok"}
